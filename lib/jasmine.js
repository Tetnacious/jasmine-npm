...
Jasmine.prototype.loadSpecs = async function() {
  ...
  await Promise.all(this.esmSpecFiles.map(file => import(`file:///${file}`)));
};
...
Jasmine.prototype.loadHelpers = async function() {
  ...
  await Promise.all(this.esmHelperFiles.map(file => import(`file:///${file}`)));
};
...
Jasmine.prototype.loadConfig = function(config) {
  ...
  if(config.esm_helpers) {
    this.addEsmHelperFiles(config.esm_helpers);
  }
  ...
  if(config.esm_spec_files) {
    this.addEsmSpecFiles(config.esm_spec_files);
  }
};
...
Jasmine.prototype.addEsmHelperFiles = addFiles('esmHelperFiles');
Jasmine.prototype.addEsmSpecFiles = addFiles('esmSpecFiles');
