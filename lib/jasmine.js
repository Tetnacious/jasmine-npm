...
Jasmine.prototype.loadSpecs = async function() {
  ...
  await Promise.all(this.ejsSpecFiles.map(file => import(`file:///${file}`)));
};
...
Jasmine.prototype.loadHelpers = async function() {
  ...
  await Promise.all(this.ejsHelperFiles.map(file => import(`file:///${file}`)));
};
...
Jasmine.prototype.loadConfig = function(config) {
  ...
  if(config.ejs_helpers) {
    this.addEjsHelperFiles(config.ejs_helpers);
  }
  ...
  if(config.ejs_spec_files) {
    this.addEjsSpecFiles(config.ejs_spec_files);
  }
};
...
Jasmine.prototype.addEjsHelperFiles = addFiles('ejsHelperFiles');
Jasmine.prototype.addEjsSpecFiles = addFiles('ejsSpecFiles');
